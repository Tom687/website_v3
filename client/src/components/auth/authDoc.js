...
export default function AuthDoc() {
	...
	return (
		...
		>
			<TextParagrah>
				C'est une API REST de niveau 2 et un système d'authentification complet, qui permet toutes les actions relatives à l'<strong>inscription d'utilisateurs</strong> <code>users</code>, leur <strong>connexion au système</strong>, la <strong>persistance de leur session</strong>, la <strong>modification de leur mot de passe</strong>, ainsi que les opérations <strong>CRUD</strong> sur les utilisateurs <code>users</code> et les <code>sessions</code>. {/*Ce système analyse chaque requête envoyée à l'API pour y chercher des <strong>JWT</strong> dans les <strong>cookies</strong> ou dans le header <code>Authorization</code>, permettant de savoir si la requête provient d'un utilisateur connecté au système ou non et de ne retourner seulement ce que le client a le droit de voir, par rapport à son <strong>rôle</strong>.*/}
			</TextParagrah>
			<TextParagrah>
				Ce système d'authentification et d'autorisation est celui même utilisé sur ce site. Il propose un système de <strong>sessions</strong> à base de <strong>JWT</strong>. Lorsqu'une requête passe par cette API, un <strong>middleware</strong> va analyser les cookies (qui sont <strong>httpOnly</strong>) ainsi que le header <code>Authorization</code> pour y chercher ces JWT. Si ils existent, les <strong>JWT</strong> <code>accessToken</code> et <code>refreshToken</code> sont extraits de la requête et décodés, permettant de savoir si le client demandant la requête est autorisé à la faire ou non. Si aucun JWT n'est trouvé ou qu'ils ne correspondent pas au client effectuant la requête, ce dernier n'est pas autorisé à voir le contenu.
			</TextParagrah>
			<TextParagrah>
				Ces JWT sont retournés au client lorsqu'une tentative de connexion sur <code>POST /me/sessions</code> est réussie (ou encore <code>GET /sessions/oauth/google</code>). Ils y sont insérés par le biais de <strong>cookies</strong> ou alors dans la mémoire du navigateur (<code>localStorage</code>). Le middleware va aussi permettre la <strong>persistance de la session utilisateur</strong> grâce à l'<code>accessToken</code> couplé au <code>refreshToken</code>. Tant que le <code>refreshToken</code> du client est valide, si l'<code>accessToken</code> ne l'est plus, il sera remplacé par un nouveau.
			</TextParagrah>
			<TextParagrah>
				De plus, l'API propose un middleware <code>requireUser</code>, qui permet de protéger les routes voulues des utilisateurs non connectés au système. Un autre middleware <code>restrictTo</code> permet de définir les rôles des utilisateurs <code>users</code> qui auront accès aux ressources du endpoint.
			</TextParagrah>
			<TextParagrah>
				On peut aussi <strong>se connecter via Google</strong>, en utilisant leur service <strong>Oauth</strong>. Lorsque ce cas ce produit et si aucun utilisateur <code>users</code> n'a la même adresse email, un utilisateur <code>users</code> sera alors créé. Dans le cas contraire, les informations de l'utilisateur <code>users</code> ne seront que mises à jour, si celles provenant de Google diffèrent de celles déjà présentes dans la base de données.
				Si on se connecte par Google, une nouvelle session de l'utilisateur est créé et les <strong>accessToken</strong> et <strong>refreshToken</strong> utilisés sont remplacés par ceux de Google.
			</TextParagrah>
			<TextParagrah>
				La base de donnée utilisée est <strong>PostgreSQL</strong> et comporte trois tables : <code>sessions</code>, <code>users</code> et <code>login</code>. La table <code>login</code> est utilisée pour enregistrer les identifiants "locaux", c'est à dire ceux des utilisateurs qui s'inscrivent via leur adresse email. La table <code>sessions</code> permet d'enregistrer toutes les <strong>sessions</strong> des utilisateurs <code>users</code>, ce qui permet la <strong>persistance de la session utilisateur</strong>, en conjonction des autres fonctionnalités de cette API.
			</TextParagrah>
			<TextParagrah>
				Comme indiqué sur le schéma ci-dessous, il existe une relation <strong>one-to-many</strong> entre la table <code>users</code> et la table <code>sessions</code>. Ainsi, un utilisateur peut avoir plusieurs sessions, mais une session ne peut être liée qu'à un seul utilisateur. De plus, une relation <strong>one-to-one</strong> exite entre <code>users</code> et <code>login</code>. De ce fait, un utilisateur ne peut avoir qu'un seul identifiant, s'il s'inscrit avec son adresse email. Cela ne l'empêche cependant pas de s'inscrire avec son adresse email, se connecter par la suite avec Google et ainsi de suite, grâce à la relation <strong>one-to-many</strong> entre les tables <code>users</code> et <code>sessions</code> et les fonctionnalités internes de l'API.
			</TextParagrah>
