...
export default function CalendarDoc() {
	...
	return (
		...
		>
			<TextParagrah>
				Calendrier utilisant <strong>React</strong> avec la bibliothèque <strong>Fullcalendar</strong>, permettant de créer l'interface du calendrier. L'interfance de démonstration donnée ici est simplement un "calendrier personnel". Cependant, grâce à toutes les fonctionnalités de l'API, on peut créer des calendriers beaucoup plus complexes.
			...
			<TextParagrah>
				L'<strong>API REST</strong> permet d'effectuer toutes les actions <strong>CRUD</strong> sur les évènements <code>events</code>, mais aussi de lier des <code>users</code> à des <code>events</code>, afin de pouvoir créer des calendriers plus complexes.
			</TextParagrah>
			<TextParagrah>
				La base de données est <strong>PostgreSQL</strong> et contient trois tables : <code>users</code>, <code>events</code> et <code>users_events</code>. Il existe une relation <strong>many-to-many</strong> entre les tables <code>users</code> et <code>events</code>, qui passe par la <strong>table de jointure</strong> <code>users_events</code>.{/*Cela permet de créer des calendriers complexes où un évènement, admettons un RDV chez une ésteticienne, peut être lié à l'employé <code>users</code> qui va s'occuper d'un ou plusieurs clients <code>users</code>. Ainsi, les clients savent quel employé va s'occuper d'eux et vice-versa. Tout ça est basé sur le role de chaque utilisateur <code>users</code> et peut être pris en compte par le système facilement. */}
			</TextParagrah>
			<TextParagrah>
				Grâce à cela, on peut créer des calendriers complexes. Prenons exemple sur un salon de beauté, dont les employés sont des utilisateurs <code>users</code> avec le <strong>role</strong> <code>staff</code>, les managers ont le <strong>role</strong> <code>manager</code> et les clients ont le <strong>role</strong> <code>client</code>. On peut imaginer un calendrier ou chaque <code>users</code> ayant le role <code>manager</code> peut voir et créer les plannings de chaque <code>users</code> ayant le role <code>staff</code>, en se basant sur les réservations effectuées par les clients ayant le role <code>client</code>, tout en ayant la possibilité de filtrer le planning de quel employé <code>staff</code> ils veulent pouvoir voir et / ou modifier.
			</TextParagrah>
			<TextParagrah>
				En reprenant l'exemple ci-dessus, chaque <code>client</code> aura sur son interface un calendrier simple, sur lequel il ne verra que les creneaux disponibles et pourra prendre RDV sur ces derniers. Lorsqu'un nouveau RDV est créé par un <code>client</code>, un email est envoyé aux <code>managers</code>, qui vont ensuite s'occuper de lier l'employé <code>staff</code> qui s'occupera du RDV du client. Une fois le lien effectué, le <code>client</code> verra sur son RDV le nom de l'employé <code>staff</code> qui s'occupera de lui, et vise-versa.
			</TextParagrah>
		</DocLayout>
	)
}