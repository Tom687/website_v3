...
export const createSession = async (userId, email, name, role, userAgent) => {
  const payload = {
    id_user: userId, /*email, */
    valid: true, /*name, */
    iat: new Date(Date.now()).toISOString(),
    user_agent: userAgent,
  }

  // TODO : Voir si on peut faire mieux ?
  const [session] = await db('sessions').insert(payload).returning(['id', 'id_user AS userId', 'valid', 'iat', 'user_agent AS userAgent'])
...
export const reIssueAccessToken = async ({ refreshToken }) => {
  ...
  const [session] = await db('sessions').select('*').where({ id: get(decoded, 'session') })

  if (!session || !session.valid) {
    return false
  }

  const [user] = await db('users').select('*').where({ id: session.id_user })
