...
export const forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const updatedUser = await db('login').update({
  ...
  try {
    ...
    await db('login').where({ email }).update({ reset_token: null, reset_token_expiration: null })
...
export const resetPassword = catchAsync(async (req, res, next) => {
  ...
  const [user] = await db('login').select('id', 'email', 'reset_token', 'reset_token_expiration').where({ reset_token: hashedToken })
  ...
  try {
    await db('login').update({ hash, reset_token: null, reset_token_expiration: null })

    // TODO : Supprimer toutes les sessions de l'user après modif du MDP ou juste valid: false ?
    await db('sessions').update({ valid: false }).where({ id_user: user.id })
