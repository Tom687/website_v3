...
export const insertEvent = catchAsync(async (req, res, next) => {
	...
	// TODO : Pas obligé de faire === undefined ?
	if (!event.id) {
...
export const updateEvent = catchAsync(async (req, res, next) => {
	...
	// TODO : req.cookies ou res.locals.user ? Choisir lequel si les 2 marchent ?
	//const role = req.locals.user.role;
	
	// If user role = user, can only modify is event. If admin, can modify all events
...
// TODO TODO TODO : Modifier pour faire /events/:eventId/linkUser et utiliser payload pour passer userId ?
//    => + Vérifier si user déjà lié ?
//    => + CREER UNLINK (/events/:eventId/unlinkUser)
...
export const unlinkUserFromEvent = catchAsync(async (req, res, next) => {
	...
	const unlinkUser = await db('users_events').del()
		.where({
			id_user: userId,
			id_event: id
		});
