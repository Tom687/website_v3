...
const validateUserLogin = async ({ email, password }) => {
  ...
  const [loginInfo] = await db('login').select('email', 'hash', 'role').where({ email })
  ...
  const [user] = await db('users').select('id', 'name', 'email', 'joined').where({ email })
...
export const getUserSessionsHandler = catchAsync(async (req, res, next) => {
  ...
  const [sessions] = await db('sessions').select('*').where({ id_user: userId, valid: true })

  if (!sessions) {
    return next(new AppError('getUserSessionsHandler : Session inexistante', 400))
  }

  const [user] = await db('users').select('id', 'name', 'email', 'picture', 'joined').where({ id: userId })
...
export const getUserCurrentSessionHandler = catchAsync(async (req, res, next) => {
  ...
  const [session] = await db('sessions').select('*').where({ id: sessionId, id_user: userId/*, valid: true*/ })
...
export const deleteSessionHandler = catchAsync(async (req, res, next) => {
  ...
  const updated = await db('sessions').update({ valid: false }).where({ id: sessionId })
