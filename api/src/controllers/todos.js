...
export const getAllTodos = catchAsync(async (req, res, next) => {
  const todos = await db('todos').select('id', 'id_user AS userId', 'title', 'completed', 'created_on AS createdOn', 'done_on AS doneOn').orderBy('id', 'asc')
...
export const getTodos = catchAsync(async (req, res, next) => {
  const todos = await db('todos').select('id', 'id_user AS userId', 'title', 'completed', 'created_on AS createdOn', 'done_on AS doneOn').where({ id_user: res.locals.user.id }).orderBy('id', 'asc')
...
export const toggleAllTodos = catchAsync(async (req, res, next) => {
  await db('todos').where({ id_user: res.locals.user.id }).update({ completed: req.body.completed })
